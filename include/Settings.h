#pragma once
#ifndef LDSO_TWITCH_SETTING_H_
#define LDSO_TWITCH_SETTING_H_
...
namespace ldso {
    ...
    const int SOLVER_SVD = 1;     // 0000 0001
    ...
    const float SCALE_F = 50.0f;        // (I think) is focal length
    const float SCALE_C = 50.0f;        // (I think) is principal point
    ...
    extern float setting_maxShiftWeightT; // Translation?
    extern float setting_maxShiftWeightR; // Rotation?
    extern float setting_maxShiftWeightRT; // Rotation + Translation?
    ...
    extern bool setting_forceAceptStep; // LwTV: Maybe it should be called "Accept"?
