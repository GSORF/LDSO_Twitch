#pragma once
#ifndef LDSO_TWITCH_GLOBAL_FUNCS_H_
#define LDSO_TWITCH_GLOBAL_FUNCS_H_

#include "NumTypes.h"

namespace ldso {

    // the global functions used in dso, for interpolation (i.e. linear, bilinear, cubic) 
    // and visualizations (i.e. rainbow, jet color ramps)
    namespace internal {
        ...
        EIGEN_ALWAYS_INLINE Eigen::Vector3f
        getInterpolatedElement33OverAnd(const Eigen::Vector3f *const mat, const bool *overMat, 
        ...
        EIGEN_ALWAYS_INLINE Eigen::Vector3f
        getInterpolatedElement33OverOr(const Eigen::Vector3f *const mat, const bool *overMat, 
                                        const float x, const float y, const int width, bool &over_out)
        {
            int ix = (int) x;
            int iy = (int) y;
                                        ...
                                        const float x, const float y, const int width)
        {
            ...
            float br = *(bp + width + 1); // bottom right
                                        ...
                                        const float x, const float y, const int width)
        {
            ...
            float br = (*(bp + width + 1))[0]; // bottom right
        ...
        // Cubic interpolation
        EIGEN_ALWAYS_INLINE float
        getInterpolatedElement11Cub(const float *const p, const float x)
        {
            // for x=0, this returns p[1].
            float res = p[1] + 0.5f * x * (p[2] - p[0] + x * (2.0f * p[0] - 5.0f * p[1] + 4.0f * p[2] - p[3] + x * (3.0f * (p[1] - p[2]) + p[3] - p[0])));
        ...
        getInterpolatedElement12Cub(const float *const p, const float x)
        {
            // for x=0, this returns p[1].
            ...
            float xx = x * x;
            float xxx = xx * x;
            Eigen::Vector2f res(p[1] + x * c1 + xx * c2 + xxx * c3,
        ...
        // Cubic interpolation
        EIGEN_ALWAYS_INLINE Eigen::Vector2f
        getInterpolatedElement32Cub(const Eigen::Vector3f *const p, const float x)
        {
            // for x=0, this returns p[1].
            ...
            float xx = x * x;
            float xxx = xx * x;
            Eigen::Vector2f res(p[1][0] + x * c1 + xx * c2 + xxx * c3,
                                c1 + x * 2.0f * c2 + xx * 3.0f * c3);
        ...
        getInterpolatedElement44(const Eigen::Vector4f *const mat, const float x, const float y, const int width)
        {
            ...
            Eigen::Vector4f res(dxdy * *(bp + 1 + width)
                                + (dy - dxdy) * *(bp + width)
                                + (dx - dxdy) * *(bp + 1)
                                + (1 - dx - dy + dxdy) * *(bp));
        ...
        getInterpolatedElement42(const Eigen::Vector4f *const mat, const float x, const float y, const int width)
        {
            ...
            Eigen::Vector2f res(dxdy * *(const Eigen::Vector2f *) (bp + 1 + width)
        ...
        inline Vec3f makeRainbowf3F(float id)
        {
            ...
            assert(false); /// TODO: Useless statement.
            return Vec3f(1,1,1);
        }
        ...
        inline bool eigenTestNan(const MatXX &m, std::string msg)
        {
            ...
            if(foundNan)
            {
               LOG(WARNING) << "NAN in " << msg.c_str() << ":" << endl;
            }
            return foundNan;
...
#endif // LDSO_TWITCH_GLOBAL_FUNCS_H_
