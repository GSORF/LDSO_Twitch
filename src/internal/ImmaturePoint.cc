...
namespace ldso {
    ...
    namespace internal {
        ...
        ImmaturePointStatus ImmaturePoint::traceOn(
            ...
            // step 1. Check the position of the point on the epipolar line
            // check idepthmin, closest distance
            ...
            // The furthest distance (note that it may be infinite)
            float dist;
            float uMax;
            float vMax;
            Vec3f ptpMax;   // Calculated according to the farthest distance, the projection in the current frame

            if (std::isfinite(idepth_max)) {
                // finite distance，finite max depth
                ...
                if (dist < setting_trace_slackInterval /* =2 by default */ ) {
                    // Two pixels on the epipolar line are very close together
                ...
                // any distance，idepth=0.01, so depth=100
