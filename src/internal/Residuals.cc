...
namespace ldso {
    ...
    namespace internal {
        ...
        double PointFrameResidual::linearize(shared_ptr<CalibHessian> &HCalib) {
            ...
            if (state_state == ResState::OOB) // The current state is already "out of bounds"
            {
            ...
            // Derivative of Lie algebra to xy
            Vec6f d_xi_x, d_xi_y;

            // Derivative of Calib to xy
            Vec4f d_C_x, d_C_y;

            // Derivative of xy to idepth
            float d_d_x, d_d_y;

            {
                ...
                // Derivative from xy to Lie algebra, see Lecture 14 for the form
